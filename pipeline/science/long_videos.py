...
class Long_videos:
    def __init__(self, para):
        ...
        if os.path.exists(self.note_dir + "chapters_and_keywords.json"):
            with open(self.note_dir + "chapters_and_keywords.json", 'r') as json_file:
    ...
    def dalle_image(self, prompt="KnoWhiz", model="dall-e-3", size="1024x1024", quality="standard", notes_set_number=-1, index=0, retry_on_invalid_request=True):
        ...
        try:
            ...
            if retry_on_invalid_request:
                ...
                response = fchain.invoke({'input': prompt, 'zero_shot_topic': self.course_description, 'chapter': self.chapters_list[notes_set_number]})
                prompt = response["prompt"]
                self.dalle_image(prompt="KnoWhiz", model=model, size=size, quality=quality, notes_set_number=notes_set_number, index=index, retry_on_invalid_request=False)
            else:
